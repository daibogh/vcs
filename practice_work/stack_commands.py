...
def load_l(username,project_name,branch_name):
	f = open(var.users_destination+username+"/"+project_name+ '/' + branch_name+"/.stack.txt","rb")
	local_stack = pickle.load(f)
	f.close()
	return local_stack

def dump_l(username,project_name,stack,branch_name):
	f = open(var.users_destination+username+"/"+project_name+ '/' + branch_name+"/.stack.txt","wb")
	pickle.dump(stack,f)
	f.close()
	return 0
def dump_g(project_name,stack,branch_name):
	f = open(var.global_destination+project_name+ '/' + branch_name+"/.stack.txt","wb")
	pickle.dump(stack,f)
	f.close()
	return 0
